...
public class UserLoginProfileServiceImpl implements UserLoginProfileService {
	...
	@Autowired
	RequestedUsersRepo requestedUsersRepo;
	...
	public Status verifyOtpForRegister(OtpDto dto) throws UserLoginProfileException {
		Status status = otpService.checkOtp(dto);
		...
		TempUserProfile userProfile = new TempUserProfile();
		...
		if(status.equals(Status.ACCEPTED)){
			...
			try {
				...
				RequestedUsers requestedUser = new RequestedUsers();
				requestedUser.setTempUserId(id);
				requestedUser.setStatus("PENDING");
				requestedUsersRepo.save(requestedUser);
			} catch (Exception e) {
			}
